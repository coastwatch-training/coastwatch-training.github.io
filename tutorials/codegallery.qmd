...
def resources_icon_for_row(row) -> str:
    """
    Generate an HTML anchor element linking to supplemental resource files
    for a Code Gallery tutorial, if available.

    This function inspects a row from the Code Gallery CSV to determine
    whether supplemental resource files (e.g., CSV, NetCDF) exist for the
    corresponding tutorial. A tutorial "slug" is inferred from the first
    available source file (Python, R, or Matlab). If a matching resources
    directory exists locally and contains at least one file, a clickable
    resources icon is returned that links to the folder on GitHub.

    If no valid tutorial path can be inferred, the resources directory does
    not exist, or the directory is empty, a placeholder dash ("—") is
    returned instead.

    Parameters
    ----------
    row : pandas.Series
        A single row from the Code Gallery CSV containing metadata and file
        paths for a tutorial.

    Returns
    -------
    str
        An HTML string containing an anchor element with an embedded SVG
        icon linking to the tutorial's resources folder on GitHub, or a
        dash ("—") if no resources are available.
    """
...
def render_carousel(img_cell, carousel_id):
    """
    Render an HTML image carousel for tutorial preview outputs.

    This function generates the HTML markup for a carousel of preview images
    displayed inside a Code Gallery modal. The input image specification may
    contain one or more image filenames, optionally paired with captions.
    Multiple images are rendered as carousel slides with navigation controls,
    while a single image is rendered without navigation.

    Image specifications are expected to be provided as a pipe-delimited
    string (e.g., "img1.png|img2.png"), with optional captions appended using
    a double-colon separator (e.g., "img1.png::Caption text").

    If no images are provided, a placeholder message indicating that no
    preview image is available is returned.

    Parameters
    ----------
    img_cell : str
        Pipe-delimited string specifying preview image filenames, optionally
        including captions using the "::" delimiter.
    carousel_id : str
        Unique HTML ID for the carousel instance, used to associate navigation
        controls with the correct carousel.

    Returns
    -------
    str
        An HTML string containing the rendered carousel markup, including
        slide containers, optional navigation controls, and image captions,
        or a placeholder message if no images are available.
    """
    if not img_cell:
