...
REPO_RAW_BASE = "https://github.com/coastwatch-wcn/coastwatch-training.github.io/raw/main"
...
function cgScrollTo(e, id){
  e.preventDefault();  // stops jump-to-top behavior

  const el = document.getElementById(id);
  if(!el) return;

  // update URL manually (so linking still works)
  ...
  // highlight flash
  el.classList.add("cg-highlight");

  setTimeout(() => {
    el.classList.remove("cg-highlight");
  }, 3000);
}

// handle direct URL visits (same as before but cleaner)
...
document.addEventListener("click", function(e){
  // ignore clicks on links that trigger highlighting
  if (e.target.closest(".cg-title-link")) return;

  document.querySelectorAll(".cg-highlight").forEach(el => {
    el.classList.remove("cg-highlight");
  });
});
</script>
