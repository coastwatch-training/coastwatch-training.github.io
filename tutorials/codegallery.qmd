...
def download_icon(path):
    ...
    table. The link points directly to a downloadable source file within the
    rendered website, such as a Jupyter notebook, R script, MATLAB file,
    or ArcGIS resource.

    Parameters
    ----------
    path : str
        Relative path to the downloadable tutorial source file.

    Returns
    -------
    str
        An HTML string containing a download icon link, or a dash ("—")
        if no valid file path is provided.
    """
    path = str(path).strip()
...
document.addEventListener("DOMContentLoaded", cgHighlightFromHash);
window.addEventListener("hashchange", cgHighlightFromHash);
</script>