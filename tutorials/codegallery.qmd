...
def render_carousel(img_cell, carousel_id):
    """
    img_cell: 'a.png::Caption A|b.png::Caption B' OR 'a.png|b.png'
    carousel_id: unique DOM id for this carousel
    """
    if not img_cell:
        return "<div class='text-muted'>No preview image</div>"

    parts = [x.strip() for x in str(img_cell).split("|") if x.strip()]
    if not parts:
        return "<div class='text-muted'>No preview image</div>"

    slides_html = []
    for j, part in enumerate(parts):
        # parse "file::caption" (caption optional)
        if "::" in part:
            fn, cap = part.split("::", 1)
            fn = fn.strip()
            cap = cap.strip()
            ...
            fn = part.strip()
            cap = fn  # fallback caption

        wrap_cls = "cg-slide active" if j == 0 else "cg-slide"

        # root-relative image path
        src = f"/tutorials/images/{fn}"

        slides_html.append(f"""
          <div class="{wrap_cls}">
            <figure class="cg-figure">
              <button type="button" class="cg-zoom" onclick="cgLightboxOpen('{src}', {cap!r})">
                <img src="{src}" loading="lazy" alt="{cap}">
              </button>
            </figure>
            <figcaption class="cg-caption">{cap}</figcaption>
          </div>
        """)
    ...
    if len(parts) > 1:
        nav = (
            ...
            f"  <span class='cg-count'>1 / {len(parts)}</span>"
...
<div id="cg-lightbox" class="cg-lightbox" aria-hidden="true">
  <div class="cg-lightbox-inner" role="dialog" aria-modal="true">
    <button type="button" class="cg-lightbox-close" onclick="cgLightboxClose()">✕</button>
    <img id="cg-lightbox-img" alt="">
    <div id="cg-lightbox-cap" class="cg-lightbox-cap"></div>
  </div>
</div>

<script>
function cgLightboxOpen(src, caption){
  const lb = document.getElementById("cg-lightbox");
  const img = document.getElementById("cg-lightbox-img");
  const cap = document.getElementById("cg-lightbox-cap");
...
function cgLightboxClose(){
  ...
  // clear to stop loading if user closes quickly
  img.src = "";
}
...
document.addEventListener("keydown", (e) => {
  if(e.key === "Escape") {
    ...
    const lb = document.getElementById("cg-lightbox");
    if(lb && lb.style.display === "flex") cgLightboxClose();
  }
});
</script>
