...
def view_link(path):
    """
    Use site path for HTML; use GitHub blob for Markdown so it renders nicely.
    """
    if not path:
        return ""
    p = str(path).strip()
    if p.lower().endswith(".md"):
        # GitHub renders markdown nicely on blob pages
        return f"{REPO_BLOB_BASE}{quote(p.lstrip('/'))}"
    # otherwise treat as a normal site link (root-relative)
...
def render_carousel(img_cell, carousel_id):
    ...
    for j, fn in enumerate(imgs):
        ...
        img_tags.append(f"<img class='{cls}' src='/tutorials/images/{fn}' loading='lazy'>")
    ...
    <div id="{mid}" class="cg-modal">
      ...
      <div class="cg-grid">
        ...
        <div>
          ...
          <p><a class="cg-btn" href="{view_link(r["r_view"])}" target="_blank" rel="noopener">View R tutorial</a></p>
        </div>
      </div>
