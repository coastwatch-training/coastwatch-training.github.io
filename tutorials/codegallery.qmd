...
REPO_RAW_BASE = "https://raw.githubusercontent.com/coastwatch-training/coastwatch-training.github.io/raw/main"
REPO_BLOB_BASE = "https://github.com/coastwatch-training/coastwatch-training.github.io/blob/main/"
REPO_TREE_BASE = "https://github.com/coastwatch-training/coastwatch-training.github.io/tree/main/"
...
<div class="cg-filterbar">
  ...
  <div class="cg-filtergroup">
    ...
    <button type="button" class="cg-chip" data-soft="arc" aria-pressed="false">
      ArcGIS <span class="cg-chipx" aria-hidden="true">×</span>
    </button>
...
def download_icon(path):
    """
    Generate an HTML anchor element that directly downloads a tutorial source file.

    This function creates a clickable download icon used in the Code Gallery
    table. When a valid file path is provided, the icon links directly to the
    raw version of the source file hosted in the GitHub repository and triggers
    a browser download instead of opening the GitHub preview page.

    The function supports downloading tutorial source files such as Jupyter
    notebooks (``.ipynb``), R scripts (``.R``), MATLAB files (``.m``),
    ArcGIS resources, or other supported code formats. If no valid path is
    provided, a placeholder dash is returned to indicate that no downloadable
    file is available.
    ...
    path : str
        ...
        The path should be relative to the repository root and compatible with
        the configured ``REPO_RAW_BASE`` URL.

    Returns
    -------
    str
        An HTML string containing an anchor element with an embedded SVG
        download icon that links directly to the raw downloadable file, or
        a dash ("—") if no valid file path is available.

    Notes
    -----
    The generated link uses the GitHub raw file endpoint rather than the
    standard GitHub blob viewer so that files download immediately when
    clicked. The HTML ``download`` attribute is included to encourage
    browser-based downloading and preserve the original filename.

    Examples
    --------
    >>> download_icon("tutorials/example/tutorial.ipynb")
    '<a href="https://raw.githubusercontent.com/...">...</a>'
    ...
    url = f"{REPO_RAW_BASE}/{quote(path)}"
...
def resources_icon_for_row(row) -> str:
    """
    Generate an HTML link element pointing to a tutorial’s resources directory.

    This function checks whether a resources folder exists locally for the
    tutorial identified by its slug and verifies that the folder contains
    at least one file. If resources are available, it returns a clickable
    icon linking to the corresponding resources directory on GitHub.
    If no resources are found, a placeholder dash is returned.

    Parameters
    ----------
    row : pandas.Series or dict-like
        A row containing a ``slug`` field that identifies the tutorial
        directory within the ``tutorials/`` folder.

    Returns
    -------
    str
        An HTML string containing a resources icon link if resources exist,
        or a dash ("—") if no resources directory or files are found.
    """
    slug = str(row.get("slug", "")).strip()
    if not slug:
        return "—"
...
def render_carousel(img_cell, carousel_id, slug):
    """
    Render an HTML image carousel for previewing tutorial outputs.

    This function parses a pipe-delimited image specification string and
    constructs an HTML carousel containing one or more preview images.
    Each image may optionally include a caption using the format
    ``filename::caption``. When multiple images are provided, navigation
    controls are added to allow users to cycle through the carousel.

    Parameters
    ----------
    img_cell : str
        A pipe-delimited string specifying image filenames and optional
        captions. Each entry may use the format ``filename::caption``.
    carousel_id : str
        A unique HTML ID assigned to the carousel container, used by
        JavaScript navigation functions.
    slug : str
        The tutorial slug used to construct relative paths to the image
        files (``{slug}/images/{filename}``).

    Returns
    -------
    str
        An HTML string containing the rendered image carousel. If no valid
        images are provided, a placeholder message indicating that no
        preview image is available is returned.
    """
    if not img_cell:
...
for i, r in df.iterrows():
    ...
    rows.append({
        ...
        "ArcGIS ↓": download_icon(r["arc_download"]),
    })
    ...
    has_arc_block  = has_any(r.get("arc_img",""), r.get("arc_view",""), r.get("arc_download",""))

    n_blocks = sum([has_py_block, has_r_block, has_m_block, has_arc_block])
    single = (n_blocks == 1)
    ...
    arc_block = ""
    if has_arc_block:
        arc_block = render_lang_block(
            "ArcGIS",
            render_carousel(r.get("arc_img", ""), f"arc-carousel-{i}", slug),
            r.get("arc_view", ""),
            single=single
        )
...
for i, r in df.iterrows():
    ...
    has_arc  = 1 if str(r.get("arc_view","")).strip()  or str(r.get("arc_download","")).strip()  else 0

    title = str(r.get("title",""))
    cat   = str(r.get("category",""))

    table_rows.append(f"""
      <tr id="cg-{slug}" class="cg-row"
          data-category="{cat.replace('"','&quot;')}"
          data-has-py="{has_py}"
          data-has-r="{has_r}"
          data-has-m="{has_m}"
          data-has-arc="{has_arc}">
        <td>
          <a href="codegallery.html#cg-{slug}"
             class="cg-title-link"
        ...
        <td>{download_icon(r.get("arc_download",""))}</td>
      </tr>
    """)
...
(function(){
  function activeSoftware(){
    const active = { py:false, r:false, m:false, arc:false };
  ...
  function cgApplyFilters(){
    ...
    const anySoft = soft.py || soft.r || soft.m || soft.arc;
    ...
    rows.forEach(row => {
      ...
      const hasArc  = row.dataset.hasArc  === "1";
